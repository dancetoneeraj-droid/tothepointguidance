/**
 * Solve Trigonometry2 questions (maths_trigonometry_156 onward), set correctAnswer
 * and write step-by-step solutions.
 *
 * Run:
 *   node scripts/add-solutions-trigonometry2.mjs
    ...
    messages: [
      {
        role: "user",
        content:
          "You are an SSC CGL Trigonometry expert. Solve this question independently.\n" +
          "Do NOT trust any pre-marked answer — compute from scratch.\n" +
          "Note: PDF formatting may be imperfect (e.g. '1/√1+k2' means 1/√(1+k²)).\n\n" +
          `Question:\n${question}\n\n` +
...
async function main() {
  const filePath = join(root, "data", "maths", "trigonometry.json");
  ...
  console.log(`Solving ${missing.length} Trigonometry2 questions (${questions[START_INDEX]?.id} …)\n`);
