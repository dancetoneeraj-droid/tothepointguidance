/**
 * Solve Race-Sheet questions (maths_time_speed_distance_191 onward),
 * set correctAnswer and write step-by-step solutions.
 *
 * Run:
 *   node scripts/add-solutions-race-sheet.mjs
    ...
    messages: [
      {
        role: "user",
        content:
          "You are an SSC CGL Time, Speed & Distance (Race) expert. Solve this question independently.\n" +
          "Do NOT trust any pre-marked answer — compute from scratch.\n" +
          "Use standard race formulas: dead heat, head start, beating distance, speed ratio.\n" +
          "Note: 'five times more than' means 6 times (original + 5 times more) in Indian exam context unless clearly additive.\n" +
          "For 'beats by X m or Y sec', speed difference = X/Y m/s.\n\n" +
          `Question:\n${question}\n\n` +
...
async function main() {
  const filePath = join(root, "data", "maths", "time-speed-distance.json");
  ...
  for (let i = START_INDEX; i < end; i++) {
    const q = questions[i];
    if (!q?.solution || q.solution.includes("Placeholder") || q.solution.includes("add real content")) {
      missing.push(i);
    }
  }

  console.log(`Solving ${missing.length} race-sheet questions (${questions[START_INDEX]?.id} …)\n`);
